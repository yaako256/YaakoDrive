...
また、Rustは、インターフェースやトレイトなどを考えて頑張って作る。
DDDやクリーンアーキテクチャを考える？
レイヤードアーキテクチャも考える？
...
Repository Trait
ここにはトレイトだけ置く。
このクレートのおかげで、SQL関連の依存関係逆転が可能となる？？
```
UserRepository
NodeRepository
...
### 使用する型
- authクレートのUser, UserId, RefreshToken, RefreshTokenId, Role
- nodeクレートのNode, NodeId, NodeType, FileContent
